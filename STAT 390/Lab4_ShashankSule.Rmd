...
Once again, the draws of $\mu_i$ are non-negative but they may not reflect the true range of ratings. The diagnostics for $\mu_1$: 

```{r}
plot(posterior, vars = "mu_j[1]")
```
The correlation for $\mu_1$ falls with lag so we must have sampled eventually with low correlation. However, the trace plot is a bit sticky so perhaps the parameter space is still unexplored. 
...
Assuming that ratings are given between $0$ and $1$, I'll use a Beta prior where the shape parameters are both log-normally sampled and stick to the standard deviation model so 

\begin{align*}
Y_{ij} &\sim N(\mu_i, \sigma) \\
\mu_i &\sim \text{Beta}(a,b) \\
\log{a},\log{b} &\sim N(\mu_0, \sigma_0)  
1/\sigma^2 \sim \\
\end{align*}

I use the values $\mu_0 = 0$, $\sigma_0 = 1$ and keep the previous values for the prior for $\sigma$. 
...
Using the beta prior, we see patterns similar to the log normal distribution except the traceplot reveals slightly lesser stickiness. However, the draws seem more correlated than the log-normal case. 
