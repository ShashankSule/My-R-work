...
Using the exact solution we have that the probability that the log total expenditure is $9$ or more is 

```{r}
1 - pnorm(9, mean = mu_n, sd = sd_n)
```

Furthermore, by sampling randomly with 1000 samples we get the probability 
...
Thus, it's very unlikely that the average log total expenditure is $9$ or more. 
...
Using the exact solution the interval is 

```{r}
qnorm(c(0.025, 0.975),mu_n,sd_n)
```

Using random sampling of 1000 samples, we get the interval to be 

```{r}
quantile(samples, probs = c(0.025,0.975))
```

Thus, the approximation using Monte Carlo simulation is quite close to the exact value (though the precision is really high so perhaps accuracy up to 4 digits is not that great anyway)
...
require(ggplot2)
S <- 1000
muvals <- rnorm(S,mu_n, sd_n)
y <- sapply(muvals, rnorm, n=1, sd=0.9)
y_pred <- data.frame(muvals,y)
ggplot(data = y_pred, aes(y_pred$y)) +
geom_histogram(breaks = seq(6, 12, by=0.1)) +
...
Calculating the probabilities we have that $Prob(\bar{y} > \bar{y}_s \mid y)$ and $1 - Prob(\bar{y} > \bar{y}_s \mid y)$ are 
...
Both probabilities are close to $0.5$, so clearly the simulated means cluster around the sample mean which implies that our likelihood and prior model describes the data well. 
